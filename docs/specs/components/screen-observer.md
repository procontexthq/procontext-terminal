...
- The canonical emulator returns protocol responses generated by its headless
  parser to the PTY in output order and remains fully interactive without a
  renderer.
...
- Queries answered by the canonical parser receive exactly one response;
  renderer projections do not echo responses already produced by it.
- Mixed response-write outcomes in one output chunk retain query occurrence
  order so a projection falls back only for the canonical write that failed.
