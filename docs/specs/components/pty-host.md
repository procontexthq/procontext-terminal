...
- Windows uses node-pty's bundled ConPTY backend so raw terminal control
  ...
  pre-model transport handshake must not depend on a renderer window or allow
  a second terminal model to answer the same query. After session construction,
  the canonical emulator owns the protocol replies generated by its headless
  parser on every platform.
