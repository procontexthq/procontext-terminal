...
sequenceDiagram
  ...
  Model-->>Session: parser callback settled + terminal responses
  Session->>PTY: terminal-generated responses
  Session->>Session: commit sequence and observation version
  Session-->>Renderer: sequenced output + ordered response outcomes
...
The canonical model owns protocol responses generated by its headless xterm
parser while processing PTY output, including device attributes and
cursor-position reports. The session writes those responses directly to the
PTY whether or not a renderer exists. Sequenced renderer output carries an
ordered outcome for every generated response, including failed PTY writes, so
each projection query remains associated with its original canonical write.
The projection consumes a query whose response was returned and may provide its
own response when the canonical write failed. Matching by query type, rather
than cursor-coordinate bytes, remains correct while canonical and projection
dimensions are temporarily converging.
